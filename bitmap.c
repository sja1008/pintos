...
struct bitmap *bitmap_expand(struct bitmap *bitmap, int size) {
	int tmp = bitmap->bit_cnt;
	bitmap->bits = realloc(bitmap->bits, byte_cnt(tmp + size));
	if (bitmap->bits == NULL) return NULL;
	bitmap->bit_cnt = tmp + size;
	bitmap_set_multiple (bitmap, tmp, size, false);
	return bitmap;
}
